...
for node in nodes:
	...
	if is_slave:
		la_cmd = "sudo sed -i 's/listen_address:.*/listen_address: {}/g' /etc/cassandra/conf/cassandra.yaml".format(ipv4)
		rpc_cmd = "sudo sed -i 's/rpc_address:.*/rpc_address: {}/g' /etc/cassandra/conf/cassandra.yaml".format(ipv4)
		...
		stdin, stdout, stderr = c.exec_command(la_cmd)
		exit_status = stdout.channel.recv_exit_status()
		command_status(exit_status, "Listen Address has been added to {}".format(dns))

		stdin, stdout, stderr = c.exec_command(rpc_cmd)
		exit_status = stdout.channel.recv_exit_status()
		command_status(exit_status, "RPC Address has been added to {}".format(dns))
		...
		command = "sudo echo '{}' | sudo tee -a /usr/local/spark/conf/spark-defaults.conf".format(spark_new_line)
